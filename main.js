...
// 创建日志文件路径
const downloadPath = process.env.DOWNLOAD_PATH || 'C:\\Users\\92175\\Downloads\\Compressed';
...
async function monitorAndExport() {
  ...
  try {
      ...
      try {
        ...
        browser = await chromium.launch({ 
          ...
          args: [
            ...
            '--use-mock-keychain',
            '--single-process', // 使用单进程模式减少内存使用
            '--js-flags="--max-old-space-size=512"' // 限制JavaScript堆内存大小
          ],
        ...
        retryCount++;
        console.error(`浏览器启动失败 (尝试 ${retryCount}/${maxRetries}):`, error);
    ...
    const context = await browser.newContext({
      viewport: { width: 1600, height: 900 }, // 调整为更合适的视窗大小
    ...
    // 使用getCookies函数获取cookies
    const cookies = getCookies();
    if (cookies.length > 0) {
      await context.addCookies(cookies);
    } else {
      console.warn('警告: 未设置cookies，可能需要手动登录');
    }
